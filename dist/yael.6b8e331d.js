...
function draw() {
    ...
    if (grassHeight < windowHeight / 2) grassHeight += 2;
    let grassColor = color(random(50, 100), random(150, 200), random(50, 100) // green brightness
    );
    if (grassScale < 5) grassScale += 0.01;
    fill(grassColor);
    for(let y = windowHeight - grassHeight; y < windowHeight; y += 10)for(let x = 0; x < windowWidth; x += 10){
        push();
        translate(x, y);
...
//# sourceMappingURL=yael.6b8e331d.js.map
