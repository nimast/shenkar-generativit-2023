...
function drawShape1(rectColorIndex) {
    ...
    rotate(angle);
    var index2;
    do index2 = Math.floor(random(CircleFills.length));
...
function drawShape(rectColorIndex) {
    ...
    if (randomShape == 0) drawShape1(rectColorIndex);
    else if (randomShape == 1) drawShape3(rectColorIndex);
    else if (randomShape == 2) drawShape2(rectColorIndex, randomGaussian(-40, 5), randomGaussian(-40, 5));
    else if (randomShape == 3) drawShape3(rectColorIndex);
    else if (randomShape == 4) drawShape1(rectColorIndex);
}
function draw() {
    ...
    for(var i = 0; i < 5; i++)for(var j = 0; j < 8; j++){
        ...
        if (angle < -5 || angle > 5) {
            ...
            var scale1;
            if (angle < 0) scale1 = map(absAngle, 5, 30, 0.25, 0.5); // Map the absolute value of the angle to a smaller scale
            else scale1 = map(absAngle, 5, 30, 0.5, 1); // Map the absolute value of the angle to a larger scale
            scale1(scale1); // Apply the scaling transformation
        }
        var rectWidth = 84; // Width of the rectangle
        var rectHeight = 108; // Height of the rectangle
        ...
        var rotatedDiagonal = diagonal * cos(rectAngle - angle) * scale1; // Calculate the length of the diagonal of the rotated rectangle
        ...
        while(CircleFills[indexCircleFill] == colorArray[index])var indexCircleFill = Math.floor(random(CircleFills.length));
        fill(CircleFills[indexCircleFill]); //Fill color
        var indexCircleStroke = Math.floor(random(CircleStroke.length));
        while(CircleStroke[indexCircleStroke] == colorArray[index])var indexCircleStroke = Math.floor(random(CircleStroke.length));
        ...
        if (d < 32) var Circle2Radius = 0;
        circle(xpos_circle2, ypos_circle2, Circle2Radius);
    }
}

//# sourceMappingURL=ori.96c10a9b.js.map
