...
function draw() {
  ...
  if (shapes.length < 6) {
    ...
    if (largeShapeCount < 3 && random() < 0.5) {
      ...
      let newShape = { type: 'circle', x, y, size };
      ...
      let newShape = { type: shapeType, x, y, size };
...
function checkCollision(newShape) {
  for (let i = 0; i < shapes.length; i++) {
    ...
    if (shape.type === 'circle' && newShape.type === 'circle') {
      ...
      if (
        newShape.x + newShape.size > shape.x &&
        newShape.x < shape.x + shape.size &&
        newShape.y + newShape.size > shape.y &&
        newShape.y < shape.y + shape.size
      ) {
        return true; // Collision detected
      }
    }
